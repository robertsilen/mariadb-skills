...
- **Autocommit (commit model is reversed)**: Oracle has no server-side `autocommit` variable and defaults to **manual commit** — a DML statement like `INSERT`/`UPDATE`/`DELETE` opens a transaction and is **not** persisted until you run an explicit `COMMIT` (a `ROLLBACK`, or DDL's implicit commit, ends it otherwise). MariaDB is the opposite: `autocommit` is **ON** by default, so each statement commits immediately. Oracle code that batches several DML statements before a single `COMMIT` will, on MariaDB, commit each one individually — losing the all-or-nothing semantics. Preserve Oracle behavior by wrapping units of work in `START TRANSACTION ... COMMIT` or by issuing `SET autocommit=0`. (Oracle's `SET AUTOCOMMIT` is only a client-side SQL\*Plus setting, not a server variable.) See Oracle's [Transactions](https://docs.oracle.com/en/database/oracle/oracle-database/21/cncpt/transactions.html) concepts guide.
